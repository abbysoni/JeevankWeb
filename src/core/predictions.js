...
export function generatePrediction(numbers) {
  if (!numbers) return "";
  ...
  if (MulyankText[m]) {
    parts.push(`Mulyank (${m}): ${MulyankText[m]}`);
  }
  if (BhagyankText[b]) {
    parts.push(`Bhagyank (${b}): ${BhagyankText[b]}`);
  }
  if (JeevankText[j]) {
    parts.push(`Jeevank (${j}): ${JeevankText[j]}`);
  }

  // You can extend later with Namank, Rashi, etc.
  return parts.join("\n\n");
}
